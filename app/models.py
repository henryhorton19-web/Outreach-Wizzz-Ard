...
class CustomVoice(BaseModel):
    ...
    # ---- Plan 26: self-learning voice type ----
    # "patch"    = Layer 4 behaviour (voice_learning nudges style sliders/notes/examples). Default,
    #              so every existing voice is byte-for-byte unchanged.
    # "exemplar" = the voice's template is INDUCED from the emails the user approved under it. Its
    #              blocks are generated by template_induct, not hand-authored. Learns only from the
    #              user's own text and edits: no reply rate, no bounce, no A/B.
    learning: Literal["patch", "exemplar"] = "patch"
    # Induced-template state for learning="exemplar". Never hand-edit these keys.
    #   {"induced_at": iso, "n_exemplars": int, "frozen": bool, "hole_effort": {hole_id: [floats]}}
...
class TargetState(BaseModel):
    ...
    # Plan 26: the per-block map compose.produce_email returned for this draft (block id -> text).
    # machine_email is "\n\n".join of these in voice order, so this map recovers exact block spans
    # inside the stored draft, which is what edit_align needs to classify an edit. Additive; {} for
    # every pre-Plan-26 draft and for authored (blank-box) emails, which have no machine draft.
    machine_blocks: dict[str, str] = Field(default_factory=dict)
