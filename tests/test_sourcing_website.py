...
bug: the harvester found no real site for 'Example SaaS', so verify.py silently produced
...
def test_no_website_produces_no_website_not_a_slug_guess():
    raw = {"name": "Example SaaS", "slug": "exsaas", "meta": {}}   # no meta["website"], no ref
...
def test_website_source_is_reported_when_present():
    raw = {"name": "Example SaaS", "slug": "exsaas", "meta": {"website": "https://example-saas.test"}}
    out = verify_candidate(raw)
...
def test_ref_is_accepted_but_marked_lower_confidence_than_meta_website():
    raw = {"name": "Example SaaS", "slug": "exsaas", "ref": "https://example-saas.test", "meta": {}}
    out = verify_candidate(raw)
    assert out["website"] == "https://example-saas.test"
